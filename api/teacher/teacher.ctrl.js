...
const list = (req, res, next) => {
    ...
    TeacherModel.find((err, result) => {
        ...
        //res.json(result);
        res.render("teacher/list", { result });
    }).limit(limit)
...
const detail = (req, res) => {
    ...
    TeacherModel.findById(id, (err, result) => {
        if(err) return res.status(500).end();
        if(!result) return res.status(404).end();
        res.render("teacher/detail", {result});
    })
};
...
const create = (req, res) => {
    const { name, birth, tel, cls } = req.body;
    if (!name || !birth || !tel || !cls) return res.status(400).send("입력값 오류입니다.");
...
// 2. MusicModel.create()
    TeacherModel.create({ name, birth, tel, cls }, (err, result) => {
...
const remove = (req, res) => {
    ...
    TeacherModel.findByIdAndDelete(id, (err, result) => {
...
const showUpdatePage = (req, res) => {
    ...
    TeacherModel.findById(id, (err, result) => {
        ...
        res.render("teacher/update", { result });
    });
};
